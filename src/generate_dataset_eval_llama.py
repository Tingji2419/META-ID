...
def main(args):
    ...
    if args.item_indexing == 'sequential':
        ...
        reindex_user_seq_dict, item_map, user_map = \
            indexing.metapath_indexing(args.data_path, args.dataset, user_sequence_dict,
                                        ...
                                        args.item_indexing, args.user_indexing, leakage=True)
    ...
    # replace("><", "> <")
    new_reindex_user_seq_dict = {}
    for user in reindex_user_seq_dict:
        items = reindex_user_seq_dict[user]
        new_reindex_user_seq_dict[user.replace("><", "> <")] = [i.replace("><", "> <") for i in items]
    ...
    if args.mode == 'validation':
        data_samples = load_validation(args, new_reindex_user_seq_dict, info)
        prompt_info = args.prompt.split(':')
        output_path = f'{args.dataset}_{args.tasks}_{args.item_indexing}_validation_{args.prompt}_llama.json'
    elif args.mode == 'test':
        data_samples = load_test(args, new_reindex_user_seq_dict, info)
        prompt_info = args.prompt.split(':')
        output_path = f'{args.dataset}_{args.tasks}_{args.item_indexing}_test_{args.prompt}_llama.json'
    else:
        raise NotImplementedError
...
def load_test(args, reindex_user_seq_dict, info):
    ...
    for user in reindex_user_seq_dict:
        ...
        if args.his_prefix > 0:
            one_sample['target'] = 'item ' + items[-1]
        else:
        ...
        if 'history' in info:
            ...
            if args.his_prefix > 0:
                one_sample['history'] = args.his_sep.join(["item " + item_idx for item_idx in history])
            else:
...
def load_validation(args, reindex_user_seq_dict, info):
    ...
    for user in reindex_user_seq_dict:
        ...
        one_sample['user_id'] = user
        if args.his_prefix > 0:
            one_sample['target'] = 'item ' + items[-2]
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt_file", type=str, default='../prompt_llama.txt', help='the path of the prompt template file')
