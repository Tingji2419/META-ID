...
def parse_args(parser):
    ...
    parser.add_argument("--gpu", type=str, default='', help="GPU device")
    ...
    # arguments related to linear
    parser.add_argument("--linear_alpha", type=float, default=0.1)
    parser.add_argument("--pad_zero_emb", type=str, default='none', help="initialize new token with zero")
...
def log_name(args):
    ...
    params = [str(args.time_str), folder_name, args.tasks, args.backbone, args.item_indexing, str(args.master_port), str(args.lr), str(args.epochs), str(args.batch_size), str(args.sample_num), str(args.seed)]
