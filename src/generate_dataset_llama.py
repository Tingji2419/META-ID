...
def main(args):
    ...
    for user in new_reindex_user_seq_dict:
        items = new_reindex_user_seq_dict[user][:-2]
        for i in range(len(items)):
            ...
            if args.his_prefix > 0:
                one_sample['target'] = 'item ' + items[i]
            else:
    ...
    output_path = f'{args.dataset}_{args.tasks}_{args.item_indexing}_train_llama.json'
