...
def plot_heatmap(item_asins, user_asins, norm_exp_rating_matrix, rating_dict, prefix='./plt/'):
    ...
    # plt.subplot(2, 1, 1)  
    sns.heatmap(sampled_ground_truth_similarity, annot=True, cmap='viridis', fmt=".2f" ,yticklabels=sampled_user_asins_sid)
    plt.title("GT")
    ...
    sns.heatmap(normalized_exp_similarity, annot=True, cmap='viridis', fmt=".2f",yticklabels=sampled_user_asins_sid)
    plt.title("Exp")
...
def plot_heatmap_sim(item_asins, experimental_similarity_items, ground_truth_similarity_items, prefix='./plt/', k=500, seed=0):
...
def main(args):
    ...
    if PLOT:
        # item_asins = sorted(list(item_emb.keys()))
        # user_asins = sorted(list(user_emb.keys()))

        # def norm_0_5(matrix):
        #     for index, i in enumerate(matrix):
        #         matrix[index] = (i - np.min(i)) / (np.max(i) - np.min(i)) * 5
        #     return matrix
        # exp_rating_matrix = np.dot(np.array([user_emb[i] for i in user_asins]), np.array([item_emb[i] for i in item_asins]).T) # [users, items]
        # norm_exp_rating_matrix = norm_0_5(exp_rating_matrix)
        # plot_heatmap(item_asins, user_asins, norm_exp_rating_matrix, rating_dict, prefix=f'./plt/{args.item_indexing}')

        plot_heatmap_sim(list(item_emb.keys()), experimental_similarity_items, ground_truth_similarity_items, prefix=f'./plt-heatmap/{args.item_indexing}', k=args.sample_nums, seed=args.seed)
