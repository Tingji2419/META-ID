...
def main():
    ...
    if 't5' in args.backbone.lower():
        ...
        tokenizer = AutoTokenizer.from_pretrained('meta-llama/' + args.backbone, trust_remote_code=True)
    else:
    ...
    def custom_collate_fn(batch):
        full_prompt = generate_prompt(batch)
        input_prompt = generate_prompt(batch, output=False)
        return {
            'input_ids': input_prompt,
            'labels': full_prompt
        }
    
    TrainSet = train_data['train'].shuffle().map(custom_collate_fn, batched=True)
    ValidSet = valid_data['train'].shuffle().map(custom_collate_fn, batched=True)
